...
export function revealReport(invoice, willShieldFirst, willShieldInline = false) {
    ...
    if (willShieldFirst) {
        items.push({
            ...
            visibility: "public",
            detail: "Shielding is a public, compliance-screened transaction: your address, the token, and the deposited amount are visible on-chain.",
        });
    }
    if (invoice.mode === "address") {
        items.push({
            ...
            visibility: "public",
            detail: `The amount (${invoice.amount} ${invoice.token}) and the address it lands on are visible. ` +
            ...
            visibility: "hidden",
            detail: "Privacy wallets submit this through the pool's relayers, so your address is not the sender of the paying transaction. " +
            ...
            visibility: "hidden",
            detail: "Each invoice uses its own address, so observers cannot total a merchant's revenue by watching one address. " +
        ...
        items.push({
            ...
            visibility: "hidden",
            detail: "A note-to-note transfer publishes only an encrypted note and a nullifier: no amount, no sender, no recipient.",
        }, {
    ...
    items.push({
        ...
        visibility: "public",
        detail: "Confirming a payment means asking a public Starknet node about this invoice's address, repeatedly, from your " +
    ...
    if (invoice.mode === "note") {
        items.push({
            ...
            visibility: "public",
            detail: "Note transfers are not discoverable on Starknet mainnet today, so a merchant cannot confirm one headlessly. " +
