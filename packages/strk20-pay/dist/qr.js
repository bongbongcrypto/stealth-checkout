...
export function rsRemainder(data, degree) {
    ...
    for (const byte of data) {
        ...
        out[degree - 1] = 0;
        for (let i = 0; i < degree; i++)
            out[i] = out[i] ^ gfMul(gen[i + 1], factor);
    }
    return out;
}
/** Data capacity in bits for a version at level M. */
function dataBits(version) {
...
function pickVersion(len) {
    for (let v = 1; v <= MAX_VERSION; v++) {
        if (4 + countBits(v) + 8 * len <= dataBits(v))
            return v;
    }
    return 0;
}
...
function buildDataCodewords(bytes, version) {
    ...
    for (let i = 0; i < bits.length; i++)
        out[i >>> 3] = out[i >>> 3] | (bits[i] << (7 - (i & 7)));
    for (let i = bits.length / 8; i < out.length; i++)
        out[i] = i % 2 === 0 ? 0xec : 0x11;
    return out;
}
...
class Builder {
    ...
    drawFormat(mask) {
        ...
        this.set(8, 7, bit(8));
        for (let i = 9; i <= 14; i++)
            this.set(8, 14 - i, bit(i));
        for (let i = 0; i <= 7; i++)
            this.set(8, n - 1 - i, bit(i));
        for (let i = 8; i <= 14; i++)
            this.set(n - 15 + i, 8, bit(i));
    }
    drawVersion() {
    ...
    applyMask(mask) {
        ...
        for (let r = 0; r < this.size; r++) {
            for (let c = 0; c < this.size; c++) {
                if (!this.fixed[r][c] && fn(r, c))
                    this.modules[r][c] = !this.at(r, c);
            }
        }
    }
    ...
    penalty() {
        ...
        const runsFinder = (line, at) => {
            for (let i = 0; i < 7; i++)
                if (line[at + i] !== FINDER[i])
                    return false;
            return true;
        };
        ...
        for (const byRow of [true, false]) {
            for (let a = 0; a < n; a++) {
                const line = [];
                for (let b = 0; b < n; b++)
                    line.push(byRow ? this.at(a, b) : this.at(b, a));
                for (let at = 0; at + 7 <= n; at++) {
                    if (!runsFinder(line, at))
                        continue;
                    if (quiet(line, at - 4, at) || quiet(line, at + 7, at + 11))
                        score += 40;
                }
            }
        }
...
export function qrDataUri(matrix, options = {}) {
