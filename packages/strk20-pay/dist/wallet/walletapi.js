 ...
 */
export function walletErrorMessage(err, seen = new Set()) {
 ...
 */
export function walletErrorCodes(err) {
    const found = [];
    const seen = new Set();
    const queue = [err];
...
export function didNotSubmit(err) {
    ...
    if (codes.some((c) => PRE_SUBMISSION_CODES.has(c)))
        return true;
...
export function userRefused(err) {
    ...
    for (const code of walletErrorCodes(err)) {
        if (code === WALLET_ERROR_CODES.USER_REFUSED_OP || code === EIP1193_USER_REJECTED)
            return true;
    }
    ...
    return /USER_(REFUSED|REJECTED|DENIED|CANCELLED|CANCELED)|user (rejected|refused|denied|declined|cancelled|canceled)|(rejected|cancelled|canceled|denied|dismissed) by (the )?user|dismissed the wallet prompt/i.test(walletErrorMessage(err));
}
