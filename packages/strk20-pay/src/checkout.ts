...
export class StealthCheckout {
  ...
  constructor(
    ...
    private readonly confirmPayment: (invoice: Invoice, txHash: string) => Promise<boolean> = alwaysConfirm,
    /**
  ...
  async pay(invoice: Invoice, opts: PayOptions = {}): Promise<Receipt> {
    ...
    try {
      ...
      if (prior && !prior.txHash) {
        ...
        if (settled) return this.finish(invoice, "", undefined);
        if (!opts.paidNothingLastTime) {
          ...
          throw new PendingPaymentError(
            "This page asked your wallet to pay this invoice earlier and never learned the outcome, and the " +
      ...
      if (didNotReachTheChain(err)) this.clearPending(invoice);
      const message =
  ...
  private markPending(invoice: Invoice): void {
    const record: SentPayment = {
      invoiceId: invoice.id,
      amount: invoice.amount,
      token: invoice.token,
      recipient: invoice.receiveAddress ?? invoice.merchantPoolAddress ?? "",
   ...
   */
  private clearPending(invoice: Invoice): void {
  ...
  private warnUnprotected(reason: string): void {
    ...
    this.emit(
      "confirming",
      `DO NOT RELOAD THIS PAGE. Your payment was sent, but it could not be remembered (${reason}), ` +
        "so reloading would pay again. Wait here until the receipt appears.",
      false,
  ...
  private emit(
    ...
    this.phase = phase;
    const progress: PaymentProgress = { phase, message, walletPopupImminent, txHash, error, severity };
...
export function shieldedNeededFor(amount: Amount, fee: Amount, decimals = 18): Amount {
 ...
 * when it is not.
 */
export function didNotReachTheChain(err: unknown): boolean {
  const raw = err instanceof Error ? err.message : String(err ?? "");
  return (
    isInsufficientFunds(err) ||
    /dismissed the wallet prompt|user (rejected|refused|denied)|USER_REFUSED|is not connected|expired|invalid|missing its receive address|wrong network|is for (mainnet|sepolia)/i.test(
      raw,
    )
  );
}
...
function defaultStore(): PaymentStore | null {
  ...
  const usable = (get: () => PaymentStore | undefined): PaymentStore | null => {
    try {
      const s = get();
      if (!s) return null;
  ...
  return (
    usable(() => (typeof localStorage === "undefined" ? undefined : localStorage)) ??
 ...
 */
const memoryStore: PaymentStore = (() => {
...
function isEphemeral(store: PaymentStore | null): boolean {
